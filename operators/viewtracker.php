...
class viewtracker
{
	...
	public function display()
	{
		...
		$this->language->add_lang('viewtracker', 'kaileymsnay/trackers');
			...
			foreach ($projects as $project_data)
			{
				...
				if ($project->project_status == constants::ITEM_ACTIVE)
				{
					...
					$this->template->assign_block_vars('projects', [
						'U_VIEWPROJECT'	=> $this->helper->route('kaileymsnay_trackers_controller', ['page' => 'viewproject', 't' => (int) $tracker_id, 'p' => (int) $project->project_id]),
		...
		$this->template->assign_vars([
			...
			'U_TRACKER_SEARCH'				=> $this->helper->route('kaileymsnay_trackers_controller', ['page' => 'search', 't' => (int) $tracker_id]),
			'U_TRACKER_MY_TICKETS'			=> $this->helper->route('kaileymsnay_trackers_controller', ['page' => 'search', 't' => (int) $tracker_id, 'assigned_user' => (int) $this->user->data['user_id']]),
			'U_TRACKER_MY_TICKETS_GROUPS'	=> $this->helper->route('kaileymsnay_trackers_controller', ['page' => 'search', 't' => (int) $tracker_id, 'assigned_user' => (int) $this->user->data['user_id'], 'include_groups' => 1]),
			'U_STATISTICS'					=> $this->helper->route('kaileymsnay_trackers_controller', ['page' => 'statistics', 't' => (int) $tracker_id]),
