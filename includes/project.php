...
class project
{
	 ...
	 * @param array                              $tables
	 */
	public function __construct(ContainerInterface $container, \phpbb\db\driver\driver_interface $db, \phpbb\user $user, $tables, $object_data = [])
	{
		...
		$this->tracker_cache = $this->container->get('kaileymsnay.trackers.tracker_cache');
	...
	{
		return new project($this->container, $this->db, $this->user, $this->tables, $object_data);
	}
	...
	{
		$this->db->sql_query('INSERT INTO ' . $this->tables['trackers_projects'] . ' ' . $db->sql_build_array('INSERT', $project_data));
	...
	public function add_team_user($user_id)
	{
		...
		$this->db->sql_query('INSERT INTO ' . $this->tables['trackers_project_auth'] . ' ' . $this->db->sql_build_array('INSERT', $sql_data));
	...
	/**
	 * Add a group to the team member list for this project
	 */
